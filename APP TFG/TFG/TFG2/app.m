function out = app2(style_in,style_ex,im_in_name,im_ex_name)

    addpath('./libs/image_pyramids/');

    input = im2double(imread(sprintf('./data/%s/imgs/%s.png', style_in, im_in_name)));
    match = im2double(imread(sprintf('./data/%s/imgs/%s.png', style_ex, im_ex_name)));

    mask_input = im2double(imread(sprintf('./data/%s/masks/%s.png', style_in, im_in_name)));
    mask_match = im2double(imread(sprintf('./data/%s/masks/%s.png', style_ex, im_ex_name)));

    fondo = im2double(imread(sprintf('./data/%s/bgs/%s.jpg', style_ex, im_ex_name)));
