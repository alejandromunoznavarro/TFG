...
%   end
  ...
  Output = out;
%   figure(7);
%   subplot(1,2,1); imshow(out); title('out');
%   subplot(1,2,2); imshow(imread(sprintf('../data/%s/imgs/%s.png', style_ex, im_ex_name))); title('im_ex')
...
figure(10);
subplot(1,2,1); imshow(Output); title('out');
subplot(1,2,2); imshow(out); title('ojos')
